...
function generateObstacle() {
    ...
    // Gera obstáculos em posições aleatórias na tela
...
function render() {
    ...
    for (let i = 0; i < gameState.lanes.length; i++) {
        ctx.beginPath();
        ctx.moveTo(gameState.lanes[i], 0);
        ctx.lineTo(gameState.lanes[i], canvas.height);
        ctx.stroke();
    }
    */
    ...
    console.log(gameState.player.acceleration);
    ctx.fillStyle = '#3498db';
    ctx.fillRect(gameState.player.position.x, gameState.player.position.y, gameState.player.width, gameState.player.height);
